...
void FG_eval::operator()(ADvector & fg, const ADvector & vars){
    ...
    for(j=0; j<p.N-1; j++){
        ...
        fg[0] += p.R(0,0)*CppAD::pow(vars[p.u1Start+j], 2);
        fg[0] += p.R(1,1)*CppAD::pow(vars[p.u2Start+j], 2);
    }
    ...
    for(j=0; j<p.N-1;j++){
        ...
        AD<double> u2_0 = vars[p.u2Start + j];
        ...
        AD<double> fx1 = p.A(0,0)*x1_0+p.A(0,1)*x2_0+p.B(0,0)*u1_0+p.B(0,1)*u2_0;
        AD<double> fx2 = p.A(1,0)*x1_0+p.A(1,1)*x2_0+p.B(1,0)*u1_0+p.B(1,1)*u2_0;
